...
export default function SubtipoAnaliticaPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [{ data: productsData }, { data: salesData }] = await Promise.all([
        supabase
          .from('products')
          .select('ean, store_id, subtipo, pvp, precio_compra')
  ...
  const subtipoRows: SubtipoRow[] = useMemo(() => {
    ...
    for (const s of filteredSales) {
      ...
      const subtipo = (product?.subtipo || '').trim() || '(sin subtipo)';
      const key = `${s.store_id}|${subtipo}`;
    ...
    const filtered = term
      ? rows.filter((r) => r.subtipo.toLowerCase().includes(term))
      : rows;
  ...
  const totalVentas = useMemo(
    () => subtipoRows.reduce((s, r) => s + r.ventas, 0),
    [subtipoRows]
  );
  const totalUnidades = useMemo(
    () => subtipoRows.reduce((s, r) => s + r.unidades, 0),
    [subtipoRows]
  );
  const numSubtipos = subtipoRows.length;
  const maxVentas = useMemo(
    () => subtipoRows.reduce((m, r) => Math.max(m, r.ventas), 0),
    [subtipoRows]
  );
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <PageHeader
          ...
          actions={<><ExportButton
            filenameBase={`analitica_subtipo_${granularidad}`}
            headers={['Subtipo', 'Tienda', 'Importe (€)', 'Unidades', 'Productos', 'PVP medio (€)', 'Margen (€)', 'Tickets']}
            rows={subtipoRows.map((r) => [r.subtipo, r.storeName, r.ventas, r.unidades, r.numProductos, r.pvpMedio, r.margenTotal, r.tickets])}
          /></>}
        />
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid md:grid-cols-3 gap-3">
              ...
              <div className="bg-cartistry-surface border border-cartistry-border rounded p-4">
                <p className="text-xs text-cartistry-text-secondary font-medium uppercase tracking-wider">Subtipos con ventas</p>
                <p className="text-2xl font-serif font-bold text-cartistry-text mt-1">{fmtInt(numSubtipos)}</p>
              </div>
            </div>
            ...
            <div className="bg-cartistry-surface border border-cartistry-border rounded overflow-hidden">
              <div className="px-4 py-3 border-b border-cartistry-border bg-cartistry-bg">
                ...
                <p className="text-xs text-cartistry-text-secondary mt-0.5">
                  Cada fila agrega las ventas de los productos del mismo subtipo, ordenadas por importe.
                </p>
              </div>

              {subtipoRows.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-surface">
                      <tr>
                        <th className="eyebrow text-left font-normal px-4 py-2.5">Subtipo</th>
                    ...
                    <tbody>
                      {subtipoRows.map((r) => {
                        ...
                        return (
                          <tr key={`${r.storeName}-${r.subtipo}`} className="border-t border-cartistry-border">
                            <td className="px-4 py-2 font-medium text-cartistry-text">{r.subtipo}</td>
