...
export default function ColeccionAnaliticaPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [{ data: productsData }, { data: salesData }] = await Promise.all([
        supabase
          .from('products')
          .select('ean, store_id, coleccion, pvp, precio_compra')
  ...
  // Agrupar ventas por (store_id + coleccion)
  const coleccionRows: ColeccionRow[] = useMemo(() => {
    ...
    for (const s of filteredSales) {
      ...
      const coleccion = (product?.coleccion || '').trim() || '(sin colección)';
      const key = `${s.store_id}|${coleccion}`;
    ...
    const filtered = term
      ? rows.filter((r) => r.coleccion.toLowerCase().includes(term))
      : rows;
  ...
  const totalVentas = useMemo(
    () => coleccionRows.reduce((s, r) => s + r.ventas, 0),
    [coleccionRows]
  );
  const totalUnidades = useMemo(
    () => coleccionRows.reduce((s, r) => s + r.unidades, 0),
    [coleccionRows]
  );
  const numColecciones = coleccionRows.length;
  const maxVentas = useMemo(
    () => coleccionRows.reduce((m, r) => Math.max(m, r.ventas), 0),
    [coleccionRows]
  );
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <PageHeader
          ...
          actions={<><ExportButton
            filenameBase={`analitica_coleccion_${granularidad}`}
            headers={['Colección', 'Tienda', 'Importe (€)', 'Unidades', 'Productos', 'PVP medio (€)', 'Margen (€)', 'Tickets']}
            rows={coleccionRows.map((r) => [r.coleccion, r.storeName, r.ventas, r.unidades, r.numProductos, r.pvpMedio, r.margenTotal, r.tickets])}
          /></>}
        />
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid md:grid-cols-3 gap-3">
              ...
              <div className="bg-cartistry-surface border border-cartistry-border rounded p-4">
                <p className="text-xs text-cartistry-text-secondary font-medium uppercase tracking-wider">Colecciones con ventas</p>
                <p className="text-2xl font-serif font-bold text-cartistry-text mt-1">{fmtInt(numColecciones)}</p>
              </div>
            </div>
            ...
            <div className="bg-cartistry-surface border border-cartistry-border rounded overflow-hidden">
              <div className="px-4 py-3 border-b border-cartistry-border bg-cartistry-bg">
                ...
                <p className="text-xs text-cartistry-text-secondary mt-0.5">
                  Cada fila agrega las ventas de los productos con la misma colección, ordenadas por importe.
                </p>
              </div>

              {coleccionRows.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-surface">
                      <tr>
                        <th className="eyebrow text-left font-normal px-4 py-2.5">Colección</th>
                    ...
                    <tbody>
                      {coleccionRows.map((r) => {
                        ...
                        return (
                          <tr key={`${r.storeName}-${r.coleccion}`} className="border-t border-cartistry-border">
                            <td className="px-4 py-2 font-medium text-cartistry-text">{r.coleccion}</td>
