...
type TramoKey = 'excelente' | 'bueno' | 'aceptable' | 'bajo' | 'sin_dato';

const TRAMOS: Array<{ key: TramoKey; label: string; range: string; color: string }> = [
  { key: 'excelente', label: 'Excelente', range: '≥ 50%',  color: '#3F7D5A' },
  { key: 'bueno',     label: 'Bueno',     range: '30–50%', color: '#7DA86F' },
  { key: 'aceptable', label: 'Aceptable', range: '15–30%', color: '#C9892F' },
  { key: 'bajo',      label: 'Bajo',      range: '0–15%',  color: '#D97A3A' },
  { key: 'sin_dato',  label: 'Sin stock conocido', range: '—', color: '#8A8073' },
];

function tramoForST(stPct: number | null): TramoKey {
  if (stPct === null) return 'sin_dato';
  if (stPct >= 50) return 'excelente';
  if (stPct >= 30) return 'bueno';
  if (stPct >= 15) return 'aceptable';
  return 'bajo';
}
...
export default function SellThroughAnaliticaPage() {
  ...
  // Paso 1: calcular ventas por producto en el periodo + ST% por producto
  const productMetrics = useMemo(() => {
    // Agregamos unidades vendidas por (store_id, ean) en el periodo
    const sold = new Map<string, number>();
    for (const s of filteredSales) {
      const key = `${s.store_id}|${s.ean}`;
      sold.set(key, (sold.get(key) || 0) + (Number(s.unidades_vendidas) || 0));
    }
    // Calculamos ST% y tramo por producto
    const productInfo = new Map<string, { tramo: TramoKey; stPct: number | null }>();
    for (const [key, soldUnits] of Array.from(sold.entries())) {
      const product = productByKey.get(key);
      if (!product) {
        productInfo.set(key, { tramo: 'sin_dato', stPct: null });
        continue;
      }
      const stock = Number(product.unidades) || 0;
      const initial = stock + soldUnits;
      const stPct = initial > 0 ? (soldUnits / initial) * 100 : null;
      productInfo.set(key, { tramo: tramoForST(stPct), stPct });
    }
    return productInfo;
  ...
  const tramoRows: TramoRow[] = useMemo(() => {
    const groups = new Map<
      string,
      {
        ...
        stWeightedSum: number;
        stWeightedCount: number;
      }
    >();
    ...
    for (const s of filteredSales) {
      ...
      if (info?.stPct !== null && info?.stPct !== undefined) {
        bucket.stWeightedSum += info.stPct * unidades;
        bucket.stWeightedCount += unidades;
      }
    }
    ...
    for (const b of Array.from(groups.values())) {
      ...
      const stMedioPct =
        b.stWeightedCount > 0 ? b.stWeightedSum / b.stWeightedCount : 0;
      rows.push({
        key: b.key,
        label: tramo.label,
        range: tramo.range,
  ...
  // Sell-through global = unidades vendidas / (unidades vendidas + stock en productos vendidos)
  const stGlobal = useMemo(() => {
    let soldTotal = 0;
    let stockTotal = 0;
    for (const [key, info] of Array.from(productMetrics.entries())) {
      const product = productByKey.get(key);
      if (!product) continue;
      // unidades vendidas de este producto en el periodo (las recalculamos)
      ...
      // info usada solo para forzar dependencia
      if (info) {
        // no-op
      }
    }
    const init = soldTotal + stockTotal;
    return init > 0 ? (soldTotal / init) * 100 : 0;
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-end justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-serif font-bold text-cartistry-text mt-2">
              Analítica · Sell-through
            </h1>
          </div>
          <ExportButton
            filenameBase={`analitica_sell-through_${granularidad}`}
            headers={['Tramo', 'Rango', 'Tienda', 'Importe (€)', 'Unidades', 'Productos', 'Margen total (€)', 'ST medio (%)', 'Tickets']}
            rows={tramoRows.map((r) => [r.label, r.range, r.storeName, r.ventas, r.unidades, r.numProductos, r.margenTotal, r.key === 'sin_dato' ? '' : r.stMedioPct.toFixed(1), r.tickets])}
          />
        </div>
      </header>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid md:grid-cols-4 gap-3">
              ...
              <div className="bg-cartistry-surface border border-cartistry-border rounded p-4">
                <p className="text-xs text-cartistry-text-secondary font-medium uppercase tracking-wider">Sell-through global</p>
                <p className="text-2xl font-serif font-bold text-cartistry-text mt-1">{fmtPct(stGlobal)}</p>
              </div>
            </div>
            ...
            <div className="bg-cartistry-surface border border-cartistry-border rounded overflow-hidden">
              <div className="px-4 py-3 border-b border-cartistry-border bg-cartistry-bg">
                <h3 className="font-serif font-bold text-cartistry-text">
                  Ventas por tramo de sell-through
                </h3>
                <p className="text-xs text-cartistry-text-secondary mt-0.5">
                  ST por producto = unidades vendidas en el periodo ÷ (vendidas + stock actual). Cada venta cae en el tramo de su producto.
                </p>
              </div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-cartistry-bg/50">
                      <tr className="text-xs text-cartistry-text-secondary">
                        ...
                        <th className="text-right px-4 py-2 font-medium">ST medio</th>
                      ...
                      {tramoRows.map((r, idx) => {
                        ...
                        return (
                          <tr key={`${r.key}-${r.storeName}-${idx}`} className="border-t border-cartistry-border">
                            ...
                            <td className="px-4 py-2 text-right text-cartistry-text-secondary">
                              {r.key === 'sin_dato' ? '—' : fmtPct(r.stMedioPct)}
                            </td>
                            <td className="px-4 py-2 text-right text-cartistry-text-secondary">
                              {fmtInt(r.tickets)}
                            </td>
