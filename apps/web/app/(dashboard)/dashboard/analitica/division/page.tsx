...
export default function DivisionAnaliticaPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [{ data: productsData }, { data: salesData }] = await Promise.all([
        supabase
          .from('products')
          .select('ean, store_id, division, pvp, precio_compra')
  ...
  const divisionRows: DivisionRow[] = useMemo(() => {
    ...
    for (const s of filteredSales) {
      ...
      const division = (product?.division || '').trim() || '(sin división)';
      const key = `${s.store_id}|${division}`;
    ...
    const filtered = term
      ? rows.filter((r) => r.division.toLowerCase().includes(term))
      : rows;
  ...
  const totalVentas = useMemo(
    () => divisionRows.reduce((s, r) => s + r.ventas, 0),
    [divisionRows]
  );
  const totalUnidades = useMemo(
    () => divisionRows.reduce((s, r) => s + r.unidades, 0),
    [divisionRows]
  );
  const numDivisiones = divisionRows.length;
  const maxVentas = useMemo(
    () => divisionRows.reduce((m, r) => Math.max(m, r.ventas), 0),
    [divisionRows]
  );
  ...
  return (
    <main className="px-6 py-10 lg:px-10 lg:py-12">
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <PageHeader
          label="Analítica"
          title="División de producto"
          actions={<><ExportButton
            filenameBase={`analitica_division_${granularidad}`}
            headers={['División', 'Tienda', 'Importe (€)', 'Unidades', 'Productos', 'PVP medio (€)', 'Margen (€)', 'Tickets']}
            rows={divisionRows.map((r) => [r.division, r.storeName, r.ventas, r.unidades, r.numProductos, r.pvpMedio, r.margenTotal, r.tickets])}
          /></>}
        />
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid md:grid-cols-3 gap-3">
              ...
              <div className="bg-cartistry-surface border border-cartistry-border rounded p-4">
                <p className="text-xs text-cartistry-text-secondary font-medium uppercase tracking-wider">Divisiones con ventas</p>
                <p className="text-2xl font-serif font-bold text-cartistry-text mt-1">{fmtInt(numDivisiones)}</p>
              </div>
            </div>
            ...
            <div className="bg-cartistry-surface border border-cartistry-border rounded overflow-hidden">
              <div className="px-4 py-3 border-b border-cartistry-border bg-cartistry-bg">
                <h3 className="font-serif font-bold text-cartistry-text">
                  Ventas por división de producto
                </h3>
                <p className="text-xs text-cartistry-text-secondary mt-0.5">
                  Cada fila agrega las ventas de los productos con la misma división, ordenadas por importe.
                </p>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-surface">
                      <tr>
                        <th className="eyebrow text-left font-normal px-4 py-2.5">División</th>
                    ...
                    <tbody>
                      {divisionRows.map((r) => {
                        ...
                        return (
                          <tr key={`${r.storeName}-${r.division}`} className="border-t border-cartistry-border">
                            <td className="px-4 py-2 font-medium text-cartistry-text">{r.division}</td>
