...
export default function ProductosAnaliticaPage() {
  ...
  useEffect(() => {
    ...
    // si es 'personalizado', no tocamos las fechas
  }, [granularidad]);
  ...
  // Agrupar ventas por producto (store_id + ean)
  const productRows: ProductRow[] = useMemo(() => {
    const groups = new Map<
      string,
      { unidades: number; ventas: number; tickets: Set<string> }
    >();
    ...
    for (const s of filteredSales) {
      ...
      if (!bucket) {
        bucket = { unidades: 0, ventas: 0, tickets: new Set<string>() };
    ...
    for (const [key, b] of Array.from(groups.entries())) {
      const [sId, ean] = key.split('|');
      const product = productByKey.get(key);
      const nombre = product?.nombre || `(EAN ${ean})`;
      ...
      const margenTotal = product
        ? ((Number(product.pvp) || pvpMedio) - precioCompra) * b.unidades
        : 0;
      rows.push({
        ean,
        nombre,
  ...
  const totalVentas = useMemo(
    () => productRows.reduce((s, r) => s + r.ventas, 0),
    [productRows]
  );
  const totalUnidades = useMemo(
    () => productRows.reduce((s, r) => s + r.unidades, 0),
    [productRows]
  );
  const productosVendidos = productRows.length;
  const maxVentas = useMemo(
    () => productRows.reduce((m, r) => Math.max(m, r.ventas), 0),
    [productRows]
  );
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <PageHeader
          ...
          actions={<><ExportButton
            filenameBase={`analitica_productos_${granularidad}`}
            headers={['Producto', 'EAN', 'Tienda', 'Importe (€)', 'Unidades', 'PVP medio (€)', 'Margen (€)', 'Tickets']}
            rows={productRows.map((r) => [r.nombre, r.ean, r.storeName, r.ventas, r.unidades, r.pvpMedio, r.margenTotal, r.tickets])}
          /></>}
        />
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid md:grid-cols-3 gap-3">
              ...
              <div className="bg-cartistry-surface border border-cartistry-border rounded p-4">
                <p className="text-xs text-cartistry-text-secondary font-medium uppercase tracking-wider">Productos vendidos</p>
            ...
            <div className="bg-cartistry-surface border border-cartistry-border rounded overflow-hidden">
              <div className="px-4 py-3 border-b border-cartistry-border bg-cartistry-bg">
                ...
                <p className="text-xs text-cartistry-text-secondary mt-0.5">
                  {productosVendidos} producto{productosVendidos === 1 ? '' : 's'} con ventas en el periodo seleccionado, ordenados por importe.
                </p>
              </div>
              ...
              {productRows.length === 0 ? (
                <div className="p-8 text-center">
                  <p className="text-cartistry-text-secondary text-sm">
                    No hay ventas de productos con estos filtros.
                  </p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-surface">
                      <tr>
                        <th className="eyebrow text-left font-normal px-4 py-2.5">Producto</th>
                        <th className="eyebrow text-left font-normal px-4 py-2.5">EAN</th>
                    ...
                    <tbody>
                      {productRows.map((r) => {
                        ...
                        return (
                          <tr key={`${r.storeName}-${r.ean}`} className="border-t border-cartistry-border">
                            <td className="px-4 py-2 font-medium text-cartistry-text">{r.nombre}</td>
                            <td className="px-4 py-2 text-xs font-mono text-cartistry-text-secondary">{r.ean}</td>
