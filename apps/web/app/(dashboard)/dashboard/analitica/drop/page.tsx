...
export default function DropAnaliticaPage() {
  ...
  // (store_id, ean) -> product (contains drop)
  ...
  // Agrupar ventas por (store_id + drop)
  const dropRows: DropRow[] = useMemo(() => {
    ...
    for (const s of filteredSales) {
      ...
      const drop = (product?.drop || '').trim() || '(sin drop)';
      const key = `${s.store_id}|${drop}`;
    ...
    const filtered = term
      ? rows.filter((r) => r.drop.toLowerCase().includes(term))
      : rows;
  ...
  const totalVentas = useMemo(
    () => dropRows.reduce((s, r) => s + r.ventas, 0),
    [dropRows]
  );
  const totalUnidades = useMemo(
    () => dropRows.reduce((s, r) => s + r.unidades, 0),
    [dropRows]
  );
  const numDrops = dropRows.length;
  const maxVentas = useMemo(
    () => dropRows.reduce((m, r) => Math.max(m, r.ventas), 0),
    [dropRows]
  );
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid md:grid-cols-3 gap-3">
              ...
              <div className="bg-cartistry-surface border border-cartistry-border rounded p-4">
                <p className="text-xs text-cartistry-text-secondary font-medium uppercase tracking-wider">Drops con ventas</p>
                <p className="text-2xl font-serif font-bold text-cartistry-text mt-1">{fmtInt(numDrops)}</p>
              </div>
            </div>
            ...
            <div className="bg-cartistry-surface border border-cartistry-border rounded overflow-hidden">
              <div className="px-4 py-3 border-b border-cartistry-border bg-cartistry-bg">
                ...
                <p className="text-xs text-cartistry-text-secondary mt-0.5">
                  Cada fila agrega las ventas de los productos con el mismo drop, ordenadas por importe.
                </p>
              </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {dropRows.map((r) => {
                        ...
                        return (
                          <tr key={`${r.storeName}-${r.drop}`} className="border-t border-cartistry-border">
                            <td className="px-4 py-2 font-medium text-cartistry-text">{r.drop}</td>
